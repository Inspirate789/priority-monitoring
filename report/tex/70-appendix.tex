\chapter*{\centerline{ПРИЛОЖЕНИЕ А. ИСХОДНЫЙ КОД ПРОГРАММЫ}}
\addcontentsline{toc}{part}{\textbf{ПРИЛОЖЕНИЕ А. ИСХОДНЫЙ КОД ПРОГРАММЫ}}

\begin{lstlisting}[
	caption={Исходный код загружаемого модуля ядра},
	label=lst:source_module,
	]
...
static int checkOverflow(char *fString, char *sString, int maxSize)
{
	...
	if (sumLen >= maxSize)
	{
		printk(KERN_ERR "%s not enough space in log (%d needed but %d 
...
\begin{lstlisting}[
	caption={Исходный код программы для получения информации о процессах в пространстве пользователя},
	label=lst:source_logger,
	]
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
